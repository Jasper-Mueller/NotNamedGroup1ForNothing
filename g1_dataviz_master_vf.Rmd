...
The happiness scores and rankings use data from the Gallup World Poll. The scores are based on answers to the main life evaluation question asked in the poll. This question, known as the Cantril ladder, asks respondents to think of a ladder with the best possible life for them being a 10 and the worst possible life being a 0 and to rate their own current lives on that scale. The scores are from nationally representative samples and use the Gallup weights to make the estimates representative. The survey contains six main sectors – economic production, social support, life expectancy, freedom, absence of corruption, and generosity – contribute to making life evaluations higher in each country than they are in Dystopia. In the 2019 World Happiness Dataset we take for analysis, there are 9 variables: Overall Rank, Country or region, Score, GDP per capita, Social support, Healthy life expectancy, Freedom to make life choice, Generosity, Perception of corruption.
...
gapminder_2019_merged <- merge(gapminder_2019, happy_data_2019) %>% 
...
Next we clean the data and create food categories. The data is not in tidy format, so first we must pivot wider. 
...
We then clean the data by creating iso2c codes and merge it with the main dataset. 
...
```
We see that Switzerland, where the quote originates, follows the trend!
...
```
As we can see in the plot above, food supply is positively correlated with happiness, affirming our hypothesis. We will investigate further when we build our model whether food supply is a significant predictor of happiness. We see Colombia somewhat follows the trend. 
...
ggplot(data,aes(x=total_litres_of_pure_alcohol,y=score,size=gdpPercap, color=most_loved))+
  ...
  NULL
```
We see Germany definitely follows their own advice!! 
...
scale_fill_gradient(low = "#DFF5EF", high = "#34AC8B", na.value = "#DCDCDC")+ 
...
ggplot(index_1, aes(x=gender_equ_indicator,
  ...
  NULL
```
In the plot above, we can see that the US follows the trend as well. 
...
# ML Model based on our datasets 

After we gathered common wisdom, myths and sayings across the world, it is time to come back and put our knowledge to the test.
